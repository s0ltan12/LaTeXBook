...
The system is built upon a proven open-source stack: \textbf{Python}, \textbf{OpenCV} (video ingestion) \cite{bradski2000opencv}, \textbf{Ultralytics YOLOv8} (object detection) \cite{ultralytics_yolov8}, and \textbf{FastAPI} (backend services) \cite{fastapi_docs}. The architecture utilizes \textbf{Edge Computing} for critical inference, ensuring feasibility even in bandwidth-constrained environments.
...
\textbf{Dashboard Alerts (Emergency Incidents):} Immediate creation and display of high-priority dashboard alerts for acute, high-severity safety hazards, namely:
...
\textbf{Remote Dashboard Updates (Emergency Incidents):} WebSocket updates to the HQ Dashboard are generated for the same set of acute, high-severity hazard types (falls, PPE violations, unsafe proximity).

\textbf{Non-Emergency Ergonomic Alerts:} Posture-related ergonomic hazards, typically classified as low or medium severity, are logged as \textbf{low-severity ergonomic events} and surfaced in dashboards and reports. They are handled as analytic records rather than emergency incidents, thereby avoiding alert fatigue while still providing actionable insights for preventive ergonomics.
...
An internal ergonomic scoring mechanism, inspired by \textbf{RULA} and \textbf{REBA} \cite{mcatamney1993rula,hignett2000reba}, shall classify posture snapshots into low, medium, or high musculoskeletal risk levels.

Non-emergency \textbf{ergonomic alerts} shall be generated for prolonged high-risk postures (e.g., sustained trunk flexion beyond a configurable duration). These are logged as \textbf{low-severity events} and do not enter the emergency incident workflow.
...
\hline
Site Supervisor & Receives dashboard alerts, performs on-site intervention, logs corrective actions. \\
\hline
...
\textbf{Hazard Detection:} The Edge AI detects a "Fall". The system validates confidence, classifies severity, checks the cooldown timer, blurs faces in the frame, and uploads the event to the Backend as an emergency incident.
...
In parallel, or rather as part of the unified hazard analysis, posture-related ergonomic hazards are evaluated and assigned typically low or medium severity levels. These posture hazards generate \textbf{non-emergency, low-severity posture records} for long-term musculoskeletal risk assessment. These posture-related records are not treated as emergency incidents but instead feed ergonomic dashboards and reports for HSE teams.
...
\textbf{Users:} Stores authentication credentials, roles, and dashboard access information.
...
High/Critical severity for acute hazards (falls, PPE, proximity) → emergency incident creation.

Low/Medium severity, especially posture-related ergonomic hazards → ergonomic records and analytic logging only.

\textbf{NotificationService:} An interface for routing alert actions and API calls without coupling the core logic to a specific output channel.
...
For high-severity acute hazards (e.g., falls, PPE, proximity), it posts an incident to the \textbf{Backend API}.

For low-severity posture-related ergonomic hazards, it posts an ergonomic record to the \textbf{Backend API} asynchronously.
...
At the Edge Node, the Stream Handler ingests RTSP camera streams and provides frames to the AI Engine for unified hazard inference. The Posture Analyzer operates alongside the core inference pipeline to estimate skeletal keypoints and compute ergonomic risk indicators, which are then consolidated within the hazard evaluation flow. Prior to any persistence or transmission, the Face Blurring component enforces privacy by anonymizing detected persons. An Alert Controller orchestrates event routing, ensuring that high-severity acute hazards are recorded as emergency incidents while posture-related ergonomic risks are typically logged as low-severity records for trend analysis.

On the Backend Server, an API Gateway exposes secured endpoints and routes requests to specialized services, including Authentication, Incident Management, Ergonomic Records, and Analytics. Data persistence is handled through a dedicated Data Layer, where structured records are stored in PostgreSQL \cite{postgres_docs}, frequently accessed data is optimized using Redis cache \cite{redis_docs}, and encrypted snapshots are stored in file storage. Real-time dashboard updates are delivered through backend event and WebSocket mechanisms.
...
Within the Industrial Site, multiple IP cameras stream video over RTSP to the Edge Server (e.g., NVIDIA Jetson). The Edge node hosts the core runtime modules—stream handling, inference execution, and alert management—along with the primary AI models (YOLOv8 for detection and a lightweight pose model for posture estimation). Confirmed high-severity hazards are sent to the backend as emergency incidents for dashboard review and response.

The Cloud Infrastructure hosts the application layer responsible for orchestration and user-facing services, including the FastAPI Backend and a WebSocket server for real-time dashboard updates. Persistent storage is provided through PostgreSQL for structured incident/ergonomic records, Redis for caching and performance optimization, and dedicated file storage for encrypted snapshots. Client devices communicate with the backend using secure channels (HTTPS/WSS), receiving dashboard incident updates and accessing analytics views, including aggregated ergonomic posture risk trends.
